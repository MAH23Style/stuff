...
navLink.forEach((e) => {
    e.addEventListener("click", (x) => {
        // Only prevent default for non-"Main Page" links
        if (e.getAttribute("data-section") !== "main") {
            x.preventDefault();
            navLink.forEach((el) => {
...
function getHourElec() {
    setInterval(() => {
        ...
        if (hr > 12) hr -= 12;
        if (hr === 0) hr = 12; // Midnight is 12, not 0
        ...
        let contentHour = `${hr}:${mn}:${sc} ${suffix}`;
            ...
            navigator.geolocation.getCurrentPosition((location) => {
                ...
                fetch(`https://api.aladhan.com/v1/calendar/${date.getFullYear()}?latitude=${latitude}&longitude=${longitude}`)
                    .then(response => {
                        ...
                        let content = `
                            <li>${convertTo12HourFormat(dateData.timings.Imsak.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(dateData.timings.Fajr.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(dateData.timings.Sunrise.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(dateData.timings.Dhuhr.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(dateData.timings.Asr.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(dateData.timings.Sunset.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(dateData.timings.Maghrib.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(dateData.timings.Isha.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(dateData.timings.Midnight.split(' ')[0])}</li>`;
            ...
            .then(response => {
                ...
                for (let prayerData of prayer.data) {
                    ...
                    if (prayerData.date.gregorian.date === dateTostring) {
                        let content = `
                            <li>${convertTo12HourFormat(prayerData.timings.Imsak.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(prayerData.timings.Fajr.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(prayerData.timings.Sunrise.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(prayerData.timings.Dhuhr.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(prayerData.timings.Asr.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(prayerData.timings.Sunset.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(prayerData.timings.Maghrib.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(prayerData.timings.Isha.split(' ')[0])}</li>
                            <li>${convertTo12HourFormat(prayerData.timings.Midnight.split(' ')[0])}</li>`;
...
const searchCity = () => {
    ...
    if (cityItem.children.length === 0 ) {
        ...
        cityItem.textContent = `<p>Choose your country</p>`;
